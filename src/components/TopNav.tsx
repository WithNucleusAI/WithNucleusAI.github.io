  ...
  return (
    ...
    >
      <div className="bg-black/[0.03] dark:bg-white/[0.03] border-b border-black/[0.08] dark:border-white/[0.06]">
        <div className="flex items-center justify-between px-4 py-2 sm:px-6 sm:py-[9px]">
          <Link href="/" className="flex items-center gap-2 no-underline transition-opacity duration-200 hover:opacity-60">
            ...
            />
            <span className="text-sm font-semibold text-black dark:text-white tracking-[0.04em]">Nucleus</span>
          </Link>
            ...
            {navItems.map((item) => {
              ...
              return (
                <Link
                  key={item.href}
                  href={item.href}
                  className={`text-[13px] sm:text-[12px] px-3 sm:px-3 py-1.5 transition-colors duration-150 ${
                    isActive
